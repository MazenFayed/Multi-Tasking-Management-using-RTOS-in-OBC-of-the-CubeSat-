...
void I2C_Read(uint8_t ADDR, uint8_t *i2cBif, uint8_t NofData)
{
	...
	I2C_MasterSendDataIT(&i2cHandler,&i2cBuf[0], 1, MPUADDR,I2C_ENABLE_SR);
	//void I2C_MasterSendData(I2C_Handle_t *pI2CHandle,uint8_t *pTxbuffer, uint32_t Len, uint8_t SlaveAddr,uint8_t Sr)
	//HAL_I2C_Master_Receive(&i2cHandler, MPUADDR, i2cBif, NofData, 100);
	I2C_MasterReceiveDataIT(&i2cHandler,i2cBif,NofData,MPUADDR,I2C_ENABLE_SR);
...
void I2C_Write8(uint8_t ADDR, uint8_t data)
{
	...
	I2C_MasterSendDataIT(&i2cHandler,&i2cData[0], 1, MPUADDR,I2C_ENABLE_SR);
...
void MPU6050_Get_Accel_Scale(ScaledData_Def *scaledDef)
{
	...
	//Accel Scale data
	scaledDef->x = ((AccelRData.x)/16384.0);
	scaledDef->y = ((AccelRData.y)/16384.0);
	scaledDef->z = ((AccelRData.z)/16384.0);
}
...
void MPU6050_Get_Gyro_Scale(ScaledData_Def *scaledDef1)
{
	RawData_Def myGyroRaw;
	MPU6050_Get_Gyro_RawData(&myGyroRaw);

	//Gyro Scale data
	scaledDef1->x = (GyroRW[0])/131.0; // x-Axis
	scaledDef1->y = (GyroRW[1])/131.0; // y-Axis
	scaledDef1->z= (GyroRW[2])/131.0; // z-Axis
}
